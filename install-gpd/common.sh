...
install_local_python() {
    ...
    if [[ -x "$python_bin" ]] && python_version_ok "$python_bin"; then
        success "App-local Python already installed at $GPD_PYTHON_DIR"
        echo "$python_bin"
        return 0
    fi
    ...
    log "Downloading Python ${PBS_PYTHON} (app-local, not system-wide)..."
    local tmp_dir
    tmp_dir="$(mktemp -d)"
    ...
    success "Python ${PBS_PYTHON} installed to $GPD_PYTHON_DIR"
    echo "$python_bin"
}
...
ensure_python() {
    ...
    if [[ -x "$local_python" ]] && python_version_ok "$local_python"; then
        success "Using app-local Python at $local_python"
        echo "$local_python"
        return 0
    fi
