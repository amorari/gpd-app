...
**Status:** Production — v1.4.12 released, all platforms
...
| Repo | Branch | What |
|------|--------|------|
| `psi-oss/opencode` | `gpd` | OpenCode fork with GPD branding, welcome screen, provider config |
| `psi-oss/opencode` | `gh-pages` | Download page at `download.gpd.psi.inc` |
| `psi-oss/opencode` | `dev` | Workflow file synced here (GitHub requires it for workflow_dispatch) |
| `get-physics-done` | `main` | GPD Python package — MCP servers, commands, agents |
...
# 6. Trigger release
gh workflow run gpd-release.yml --repo psi-oss/opencode --ref gpd -f version=1.5.0
```
...
| Add new models | When providers release | LiteLLM API + fork update |
| Rebase on new OpenCode | Every few days | Cherry-pick, push, release |
| Update LiteLLM | Monthly | Railway redeploy |
| Update GPD package | When GPD ships updates | Rebuild sidecar in CI |
