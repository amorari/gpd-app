...
// Dev-only. Imported conditionally in ../../src/index.tsx under
// __GPD_TAURI_DEBUG__ so Vite tree-shakes it from release builds — matches
// the plugin's Rust-side cfg(debug_assertions) gate. Co-located with the
// pytest harness under tests-gui/ since that is the only code that depends
// on it. Refresh with:
...
//     -o packages/desktop/tests-gui/vendor/tauri-plugin-mcp.ts
