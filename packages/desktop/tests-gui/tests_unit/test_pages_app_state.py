...
def test_gpd_pid_returns_first(monkeypatch):
    monkeypatch.setattr(
        mod.subprocess, "run", lambda *a, **kw: _cp("111\n222", 0)
    )
...
def test_gpd_pid_none_when_empty(monkeypatch):
    monkeypatch.setattr(
        mod.subprocess, "run", lambda *a, **kw: _cp("", 1)
    )
...
def test_sidecar_pid_none_when_no_sidecar_running(monkeypatch):
    router = _RunRouter().bind("pgrep", lambda argv: _cp("", 1))
...
def test_sidecar_pid_returns_first_when_no_launched_parent(monkeypatch):
    ...
    monkeypatch.setattr(
        mod.subprocess, "run", lambda *a, **kw: _cp("9001\n9002", 0)
    )
...
def test_kill_stale_noop_when_already_dead(monkeypatch):
    router = _RunRouter()
    router.bind("pgrep", lambda argv: _cp("", 1))
    router.bind("kill", lambda argv: _cp("", 0))
    monkeypatch.setattr(mod.subprocess, "run", router)
    # Force wait_until to return True immediately.
...
def test_kill_stale_sigterm_sufficient(monkeypatch):
    """pgrep returns pids; wait_until succeeds → no SIGKILL escalation."""
    router = _RunRouter()
    # Round 1 returns pids for each pattern; subsequent rounds: doesn't matter
    # because we stub wait_until to say "gone".
    router.bind("pgrep", lambda argv: _cp("101\n102", 0))
    router.bind("kill", lambda argv: _cp("", 0))
    monkeypatch.setattr(mod.subprocess, "run", router)
    monkeypatch.setattr(mod, "wait_until", lambda pred, **kw: True)
    killed: list[int] = []
...
def test_kill_stale_escalates_to_sigkill(monkeypatch, capsys):
    router = _RunRouter()
    router.bind("pgrep", lambda argv: _cp("555\n556", 0))
    router.bind("kill", lambda argv: _cp("", 0))
    monkeypatch.setattr(mod.subprocess, "run", router)

    # First wait_until (post-TERM) fails; second (post-KILL) succeeds.
    state = {"n": 0}
    ...
    # Survivors for both patterns are sent SIGKILL.
...
def test_kill_stale_sigkill_swallows_process_lookup_error(monkeypatch):
    router = _RunRouter()
    router.bind("pgrep", lambda argv: _cp("900", 0))
    router.bind("kill", lambda argv: _cp("", 0))
    monkeypatch.setattr(mod.subprocess, "run", router)
    state = {"n": 0}
    ...
    monkeypatch.setattr(mod.os, "kill", raise_not_found)
    # Must not propagate the ProcessLookupError.
    mod.AppState().kill_stale()
...
def test_kill_stale_raises_when_sigkill_insufficient(monkeypatch):
    router = _RunRouter()
    router.bind("pgrep", lambda argv: _cp("200", 0))
    router.bind("kill", lambda argv: _cp("", 0))
    monkeypatch.setattr(mod.subprocess, "run", router)
    # wait_until always fails → TimeoutError.
...
def test_launch_timeout_message_includes_stale_pids(monkeypatch):
    def run(argv, *a, **kw):
        if argv[0] == "pgrep":
            return _cp("77\n88", 0)  # pids exist but predicate stub lies
        return _cp("", 0)
...
def test_wait_quit_raises_on_timeout(monkeypatch):
    monkeypatch.setattr(
        mod.subprocess, "run", lambda *a, **kw: _cp("42", 0)
    )
    monkeypatch.setattr(mod, "wait_until", lambda pred, **kw: False)
...
def test_wait_quit_success(monkeypatch):
    monkeypatch.setattr(mod.subprocess, "run", lambda *a, **kw: _cp("", 1))
    monkeypatch.setattr(mod, "wait_until", lambda pred, **kw: True)
...
def test_refresh_launched_pid_rebinds(monkeypatch):
    monkeypatch.setattr(
        mod.subprocess, "run", lambda *a, **kw: _cp("5150", 0)
    )
    s = mod.AppState()
    assert s._launched_pid is None
...
def test_wait_launched_success_rebinds_pid(monkeypatch):
    ...
    monkeypatch.setattr(
        mod.subprocess, "run", lambda *a, **kw: _cp("4242", 0)
    )
    monkeypatch.setattr(mod, "wait_until", lambda pred, **kw: True)
    s = mod.AppState()
...
def test_wait_launched_ready_predicate_paths(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        mod.subprocess,
        "run",
        lambda *a, **kw: _cp("7", 0) if running_flag["v"] else _cp("", 1),
    )
